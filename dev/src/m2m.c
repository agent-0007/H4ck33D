...
{
	/* destination pointer */
	DMAC1->CMAR = (uint32_t)dst;
	/* source pointer */
	DMAC1->CPAR = (uint32_t)src;
	/* set size */
	DMAC1->CNDTR = size;

	/* enable transfer */
	DMAC1->CCR = DMA_CCR_MEM2MEM | DMA_CCR_MINC | DMA_CCR_PINC | DMA_CCR_EN;
...
void M2M_Set(void *dst, uint32_t data, size_t size)
{
	...
	DMAC1->CMAR = (uint32_t)dst;
	/* source pointer */
	DMAC1->CPAR = (uint32_t)&data;
	/* set size */
	DMAC1->CNDTR = size;

	/* enable transfer */
	DMAC1->CCR = DMA_CCR_MEM2MEM | DMA_CCR_MINC | DMA_CCR_EN;
	/* wait till transfer finishes */
	while ((DMA1->ISR & DMA_ISR_TCIF1) == 0);
	/* disable transfer */
	DMAC1->CCR = 0;
